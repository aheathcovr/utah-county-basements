...
async def check_robots(crawler: AsyncWebCrawler) -> None:
    ...
    try:
        result = await crawler.arun(url="https://www.utahcounty.gov/robots.txt", bypass_cache=True)
        print("\n--- robots.txt ---")
...
async def collect_parcel_ids(crawler: AsyncWebCrawler) -> list[str]:
    ...
    for page in range(1, MAX_SEARCH_PAGES + 1):
        # Try GET-based search with pagination offset
        # The portal may use ?avStart= or ?avPage= for pagination — try both
        search_url = (
            f"{SEARCH_URL}"
            f"?avCity={TARGET_CITY.replace(' ', '+')}"
            f"&avType=RES"
            f"&avStart={((page - 1) * 25) + 1}"  # 25 results per page is common
        )

        log.info(f"  Fetching search page {page}: {search_url}")
        try:
            result = await crawler.arun(url=search_url, bypass_cache=True, wait_for="table")
            if not result or not result.html or len(result.html) < 500:
            ...
            log.info(f"  Page {page}: {len(new_ids)} new parcel IDs (total: {len(parcel_ids)})")

            # Stop early if we have plenty to work with
            if len(parcel_ids) >= 200:
                log.info("  Collected 200+ IDs — stopping search pagination for test run")
                break

            # If we got no new IDs, we've exhausted results
            if not new_ids:
                log.info("  No new IDs on this page — search complete")
                break

            # If no next-page indicator, stop
            html_lower = result.html.lower()
            has_next = (
                "next" in html_lower
                or f"avstart={page * 25 + 1}" in html_lower
                or len(found) >= 20  # Still getting full pages
            )
            if not has_next:
            ...
            log.error(f"  Error fetching search page {page}: {e}")
            break
...
async def scrape_until_target(
    ...
    for parcel_id in parcel_ids:
        ...
        log.info(f"  [{processed + 1}/{len(parcel_ids)}] Parcel {parcel_id} ...")
...
async def main():
    log.info(f"Starting Utah County basement scraper test run — {datetime.now()}")
    ...
    async with AsyncWebCrawler(verbose=False, headless=True) as crawler:
        ...
        if not parcel_ids:
            log.error("No parcel IDs collected — cannot proceed. Check search URL and portal structure.")
            log.info("Tip: Review the HTML printed in Phase 1 to identify the correct search form fields.")
            return
    ...
    if leads:
        ...
        log.info(f"\n{'=' * 55}")
        log.info(f"TEST RUN COMPLETE")
        log.info(f"Qualifying leads found: {len(leads)}")
        log.info(f"Output written to: {filepath}")
        log.info(f"{'=' * 55}")
        print("\nTop leads:")
        ...
        log.warning("No qualifying leads found in the test run.")
        log.info("Next steps:")
        log.info("  1. Review Phase 1 HTML output to verify parser label names")
        log.info("  2. Check Phase 2 search results — confirm parcel IDs were found")
        log.info("  3. Adjust label matching in parse_parcel_html() if needed")
